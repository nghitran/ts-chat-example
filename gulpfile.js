...
gulp.task('clean', 'Cleans the generated files from dist directory', function () {
  return del([
    'dist/*'
  ]);
});

gulp.task('copy-static-files', 'Copy static files to dist directory', function() {
...
gulp.task('build', 'Compiles all TypeScript source files and updates module references', gulpSequence('tslint', ['update-tsconfig', 'copy-static-files'], '_build'));
